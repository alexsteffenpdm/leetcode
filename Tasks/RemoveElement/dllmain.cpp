...
namespace RemoveElement {
    ...
    struct DLLSolutionInput {
      public:
        ...
        DLLSolutionInput(std::vector<int> Numbers, int Value) : nums(std::move(Numbers)), val(Value) {}
    };
    ...
    static std::ostream& operator<<(std::ostream& os, const DLLSolutionInput& output) {
        ...
        os << "  val: " << output.val << ",\n";
        os << "  nums: [";
        if (!output.nums.empty()) {
            for (size_t i = 0; i < output.nums.size(); ++i) {
                os << output.nums[i];
                if (i < output.nums.size() - 1) os << ", ";
            }
        }
        os << "]\n";
        os << "}";
    ...
    struct DLLSolutionOutput {
      public:
        ...
        DLLSolutionOutput(std::vector<int> Numbers, int Size) : nums(std::move(Numbers)), size(Size) {}
    };
    ...
    static bool operator==(const DLLSolutionOutput& lhs, const DLLSolutionOutput& rhs) {
    ...
    class SolutionTest : public TestCase<DLLSolutionInput, DLLSolutionOutput> {
      public:
        SolutionTest(DLLSolutionInput Inputs, DLLSolutionOutput Outputs)
            : TestCase<DLLSolutionInput, DLLSolutionOutput>(std::move(Inputs), std::move(Outputs)) {}

        static SolutionTest create(DLLSolutionInput Inputs, DLLSolutionOutput Outputs) {
            return SolutionTest(std::move(Inputs), std::move(Outputs));
        }
        ...
        bool validateResults(const DLLSolutionOutput& Results) const override {
            ...
            if (!Results.nums.empty()) {
                ...
                countNonOne = static_cast<int>(
                    std::count_if(Results.nums.begin(), Results.nums.end(), [max](int x) { return x != max; }));
            }

            if (countNonOne != this->ExpectedResults.size) {
                std::cout << "\t"
                          << "Test failed: size mismatch Got:\n"
                          << countNonOne << "\nExpected:\n"
                          << this->ExpectedResults << std::endl;
                return false;
            }
            // Compare the expected output nums against the first Results.size elements:
            if (std::equal(Results.nums.begin(), Results.nums.begin() + this->ExpectedResults.size,
                           this->ExpectedResults.nums.begin())) {
                std::cout << "\t"
                          << "Test passed" << '\n';
                return true;
            }
            std::cout << "\t"
                      << "Test failed: content mismatch Got:\n"
                      << Results << "\nExpected:\n"
                      << this->ExpectedResults << std::endl;
            return false;
        }
      ...
      public:
        ...
        DLLSolution() {
            ...
            Tests.push_back(SolutionTest<DLLSolutionInput, DLLSolutionOutput>::create(
                DLLSolutionInput({3, 2, 2, 3}, 3), DLLSolutionOutput({2, 2, 4, 4}, 2)));

            Tests.push_back(SolutionTest<DLLSolutionInput, DLLSolutionOutput>::create(
                DLLSolutionInput({0, 1, 2, 2, 3, 0, 4, 2}, 2), DLLSolutionOutput({0, 0, 1, 3, 4, 5, 5, 5}, 5)));

            Tests.push_back(SolutionTest<DLLSolutionInput, DLLSolutionOutput>::create(DLLSolutionInput({}, 0),
                                                                                      DLLSolutionOutput({}, 0)));
        };
         ...
         */
        DLLSolutionOutput run(DLLSolutionInput& Inputs) override { return removeElement(Inputs.nums, Inputs.val); }

        static DLLSolutionOutput removeElement(std::vector<int>& Nums, int Value) {
            if (Nums.empty()) return {{}, 0};

            int const max           = *std::max_element(Nums.begin(), Nums.end()) + 1;
            int const NonValueCount = static_cast<int>(
                std::count_if(Nums.begin(), Nums.end(), [Value](int NumValue) { return NumValue != Value; }));
            std::transform(Nums.begin(), Nums.end(), Nums.begin(),
                           [Value, max](int x) { return x == Value ? max : x; });
            std::sort(Nums.begin(), Nums.end());
            return {std::move(Nums), NonValueCount};
        }
